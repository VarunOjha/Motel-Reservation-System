...
# Run the collection
echo "🏃 Running Room API tests..."
echo ""

newman run Room-API.postman_collection.json \
    --reporters cli,json \
    --reporter-json-export results-room.json \
    --color on \
    --delay-request 100
...
if [ $? -eq 0 ]; then
    ...
    echo "📊 Results saved to: results-room.json"
else
    echo ""
    echo "❌ Some tests failed!"
    echo "📊 Check results-room.json for details"
    exit 1
fi
